...
class AgendaService extends BaseService {
    ...
    async getAll() {
        ...
        if (!Array.isArray(data)) {
            console.warn("[AgendaService] Detectado formato NO-Array. Reiniciando a []");
            return [];
        }
        this.cache = data;
        return this.cache;
    ...
    async importarDesdeTexto(textoOrArray, mergeMode = false, interactive = true) {
        console.log(`Importando/Fusionando agenda (Merge: ${mergeMode}, Interactive: ${interactive})...`);
        ...
        const CHUNK_SIZE = 50; // Process 50 contacts per tick
            ...
            return new Promise((resolve) => {
                ...
                for (let i = startIndex; i < endIndex; i++) {
                    ...
                    nuevosContactos.push({
                        ...
                        nombre: nombre,
                        telefonos,
                        email: "", web: "", direccion: {}, vinculo: "Otro", categoria: "Información",
                        comentarios: extraLines.join('. '), favorito: false
                    });
                }
                
                // Allow UI to breathe
                setTimeout(() => resolve(endIndex < bloques.length ? endIndex : null), 0);
            });
        };

        // Execution Loop
        let currentIndex = 0;
        console.log(`[AgendaService] Iniciando procesamiento de ${bloques.length} bloques...`);
        
        while (currentIndex !== null) {
            // Log progress every 10 chunks (500 items) to avoid spam
            if (currentIndex % 500 === 0) console.log(`[AgendaService] Procesando índice ${currentIndex}...`);
            currentIndex = await processChunk(currentIndex);
        }

        console.log(`[AgendaService] Procesamiento finalizado. Nuevos a añadir: ${nuevosContactos.length}`);
        ...
        if (nuevosContactos.length > 0) {
            ...
            if (mergeMode) {
                if (interactive) {
                     await Modal.showAlert(`✅ <strong>Recuperación Completada</strong><br>Se han restaurado ${nuevosContactos.length} contactos originales.<br>Datos existentes conservados.`, "success");
                     location.reload();
                } else {
                    console.log(`[Auto-Repair] ${nuevosContactos.length} contactos restaurados silenciosamente.`);
                    // Optional: Show a non-blocking toast
                    if (window.showToast) window.showToast(`Agenda reparada: +${nuevosContactos.length} contactos`, "success");
                }
            }
        } else {
            console.warn("[AgendaService] No hay contactos nuevos.");
            if (mergeMode && interactive) {
                 await Modal.showAlert(`ℹ️ <strong>Agenda al día</strong><br>No faltaba ningún contacto original.`, "info");
            }
        }
    }
}
